...
namespace Technosoftware.UaServer.Aggregates
{
    ...
    public class AggregateCalculator : IUaAggregateCalculator
    {
            ...
            AggregateConfiguration configuration)
        {
            ...
            values_ = new LinkedList<DataValue>();
        }
        #endregion
        ...
        public bool QueueRawValue(DataValue value)
        {
            ...
            if (startOfData_ == DateTime.MinValue)
            {
                startOfData_ = value.SourceTimestamp;
            }

            // update end of data.
            endOfData_ = value.SourceTimestamp;

            // ensure values are being queued in the right order.
            if (TimeFlowsBackward)
            {
                if (values_.First != null && CompareTimestamps(value, values_.First) > 0)
                {
                    return false;
                }
            }
            ...
            {
                if (values_.Last != null && CompareTimestamps(value, values_.Last) < 0)
                {
                    return false;
                }
            }
        ...
        public DataValue GetProcessedValue(bool returnPartial)
        {
            ...
            if (SetPartialBit)
            {
                if (startOfData_ > earlyTime && startOfData_ < lateTime)
                {
                ...
                {
                    if (endOfData_ >= earlyTime && endOfData_ < lateTime)
                    {
                        value.StatusCode = value.StatusCode.SetAggregateBits(value.StatusCode.AggregateBits | AggregateBits.Partial);
                    }
                }
        ...
        protected int CompareTimestamps(DataValue value1, DataValue value2)
        {
            ...
            {
                var result = value1.ServerTimestamp.CompareTo(value2.ServerTimestamp);
            ...
            {
                var result = value1.SourceTimestamp.CompareTo(value2.SourceTimestamp);
        ...
        public static DataValue SlopedInterpolate(DateTime timestamp, DataValue earlyBound, DataValue lateBound)
        {
            try
            {
                ...
                // convert to doubles.
                var earlyValue = CastToDouble(earlyBound);
                var lateValue = CastToDouble(lateBound);

                // do interpolation.
                var range = (lateBound.SourceTimestamp - earlyBound.SourceTimestamp).TotalMilliseconds;
                var slope = (lateValue - earlyValue) / range;
                var calculatedValue = slope * (timestamp - earlyBound.SourceTimestamp).TotalMilliseconds + earlyValue;
        ...
        {
            var castValue = TypeInfo.Cast(value, TypeInfo.Scalars.Double, original.WrappedValue.TypeInfo.BuiltInType);
        ...
        protected List<DataValue> GetValuesWithSimpleBounds(TimeSlice slice)
        {
            ...
            if (CompareTimestamps(slice.StartTime, values_.Last) > 0 || CompareTimestamps(slice.EndTime, values_.First) < 0)
            {
                return null;
            }
        ...
        protected List<DataValue> GetValuesWithInterpolatedBounds(TimeSlice slice)
        {
            ...
            if (CompareTimestamps(slice.EndTime, values_.First) < 0)
            {
                return null;
            }
        ...
        private LinkedList<DataValue> values_;
        private DateTime startOfData_;
        private DateTime endOfData_;
        #endregion
    }
}
