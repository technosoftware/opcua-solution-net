...
namespace Technosoftware.UaConfiguration
{
    ...
    public class ApplicationInstanceTests
    {
        ...
        public const string SubjectName = "CN=OPC UA Configuration Test, O=Technosoftware GmbH, C=CH, S=Aargau";
        ...
        public void SetUp()
        {
            ...
            pkiRoot_ = Path.GetTempPath() + Path.GetRandomFileName() + Path.DirectorySeparatorChar;
        }
        ...
        public void TearDown()
        {
            ...
            {
                // pki directory root for test runs. 
                Directory.Delete(pkiRoot_, true);
            }
            catch
            { }
        }
        ...
        public async Task TestFileConfig()
        {
            ...
            string configPath = Opc.Ua.Utils.GetAbsoluteFilePath("Technosoftware.UaConfiguration.Tests.Config.xml", true, false, false);
        ...
        public async Task TestNoFileConfigAsClient()
        {
            ...
            Assert.NotNull(applicationInstance);
            ApplicationConfiguration config = await applicationInstance.CreateApplicationConfigurationManager(ApplicationUri, ProductUri)
                .AsClient()
                .AddSecurityConfiguration(SubjectName, pkiRoot_)
        ...
        public async Task TestBadApplicationInstance()
        {
            ...
            Assert.ThrowsAsync<ServiceResultException>(async () =>
               await applicationInstance.CreateApplicationConfigurationManager(ApplicationUri, ProductUri)
            ...
            var config = await applicationInstance.CreateApplicationConfigurationManager(ApplicationUri, ProductUri)
                .AsServer(new string[] { EndpointUrl })
                .AddSecurityConfiguration(SubjectName, pkiRoot_)
                .CreateAsync()
                .ConfigureAwait(false);
        ...
        public async Task TestInvalidAppCertDoNotRecreate(InvalidCertType certType, bool server, bool suppress)
        {
            ...
            if (server)
            {
                config = await applicationInstance.CreateApplicationConfigurationManager(ApplicationUri, ProductUri)
        ...
        public async Task TestAddOwnCertificateToTrustedStore()
        {
            ...
            ApplicationConfiguration configuration = await applicationInstance.CreateApplicationConfigurationManager(ApplicationUri, ProductUri)
                .SetOperationTimeout(10000)
                .AsServer(new string[] { EndpointUrl })
                .AddSecurityConfiguration(SubjectName, pkiRoot_)
                .CreateAsync().ConfigureAwait(false);
