                    ...
                    do
                    {
                        switch (iteration)
                        {
                            ...
                            case 7:
                                ...
                                NodeId nodeId = await Session.NodeCache.FindSuperTypeAsync(SampleCompany.NodeManagers.TestData.DataTypeIds.Vector).ConfigureAwait(false);
                                Assert.AreEqual(DataTypeIds.Structure, nodeId);
                                NodeId nodeId2 = await Session.NodeCache.FindSuperTypeAsync(ExpandedNodeId.ToNodeId(SampleCompany.NodeManagers.TestData.DataTypeIds.Vector, Session.NamespaceUris)).ConfigureAwait(false);
